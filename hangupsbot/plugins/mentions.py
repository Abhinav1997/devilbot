...
def mention(bot, event, *args):
    ...
    """
    /devilbot mention <fragment> test
    """
            ...
            if event.user_id.chat_id not in admins_list:
                ...
                if all_whitelist is None or event.user_id.chat_id not in all_whitelist:
                    ...
                    if conv_1on1_initiator:
                        yield from bot.coro_send_message(
                            conv_1on1_initiator,
                            _("You are not allowed to mention all in <b>{}</b>").format(
                                conversation_name))
                    if noisy_mention_test or bot.get_config_suboption(event.conv_id, 'mentionerrors'):
                        yield from bot.coro_send_message(
                            event.conv,
                            _("<b>{}</b> blocked from mentioning <i>all</i>").format(
                                event.user.full_name))
                    return
                else:
    ...
    if len(mention_list) > 1 and username_lower != "all":
        ...
        if send_multiple_user_message or noisy_mention_test:
            if conv_1on1_initiator:
                ...
                text_html += "<br /><em>To toggle this message on/off, use <b>/devilbot bemorespecific</b></em>"
...
def pushbulletapi(bot, event, *args):
    """allow users to configure pushbullet integration with api key
        /devilbot pushbulletapi [<api key>|false, 0, -1]"""

    # XXX: /devilbot config exposes all configured api keys (security risk!)
